...
%     for i=0:Mat-1
%        frames(:,(i+1)) = single(audioData((i*64)+1 : (i*64+Nw))); 
%     end
%     
...
%     for i=1:Mat
%         frames(:,i) = abs(fft(frames(:,i),nfft, 1)); 
%     end
...
%     end
    ...
    currentFrame = single(zeros(Nw, 1)); 
    CC2 = single(zeros(C, Mat));
    %tab = single(zeros(K, 1));
    for j=1: Mat
        currentFrame = single(audioData(((j-1)*64)+1 : (j-1)*64+Nw));
    ...
    for cc_k = 1 : size(CC2,2)
         for i = 0:C-1
            CC2(i+1,cc_k) = 1+0.5*L*sin(pi*i/L) * CC2(i+1,cc_k);
         end
    end
